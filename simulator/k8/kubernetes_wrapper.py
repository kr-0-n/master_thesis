...
class Kubernetes:
    ...
    def tick(self):
        if self.current_deployment is not None:
            ...
            pods_online_count = len([node for node in self.graph.nodes if self.graph.nodes[node]["type"] == "pod" and self.graph.neighbors(node) != []])
            ...
            pods_to_schedule = [pod for pod in self.current_deployment['pods'] if pod[0] not in [node for node in self.graph.nodes if self.graph.nodes[node]["type"] == "pod"] or self.graph.neighbors(pod[0]) == []] 
